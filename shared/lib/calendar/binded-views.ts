...
import { renderIf } from '@/shared/lib/wrappers'

import {
	$daysWithFilledSideWeeks,
	$selectedMonth,
	nextMonthSelected,
	prevMonthSelected,
	getMonthName,
...
import {
	monthViewPropsConnector,
	headerPropsConnector,
} from './props-connectors'
...
export const bindedViews = {
	Header: reflect({
		view: pipe(
			Header,
			headerPropsConnector,
			renderIf(({ monthName }) => O.isSome(monthName)),
		),
		bind: {
